#include<bits/stdc++.h>
using namespace std;typedef long long o;
struct T{struct N{N*l,*r,*p,*q;o c,L,R,A,v;bool f;}*t;int n;
...
void K(int k){N*x=t;Z(x);for(;;){while(x->l&&x->l->c>k)x=x->l,Z(x);if(x->l)k-=x->l->c;if(!k--)break;x=x->r,Z(x);}S(x);}
void G(int m){n=m;N*x;t=x=new N;x->l=x->r=x->p=NULL;x->c=m;U(x);while(--m){x->r=new N;x->r->p=x;x=x->r;x->l=x->r=NULL;x->c=m;U(x);}}
void I(int l,int r){K(l-1);N*x=t;t=x->r;t->p=NULL;K(r-l+1);x->r=t;t->p=x;t=x;}
void Rv(int l,int r){N*x=t;if(l||r!=n-1){if(!l){K(r+1);x=t->l;}else if(r==n-1){K(l-1);x=t->r;}else{I(l,r);x=t->r->l;}}x->f^=1;for(;x;){U(x);x=x->p;}}
};
int main(){ios::sync_with_stdio(0);cin.tie(0);
int n,m,a,l,r;cin>>n;T s;s.G(n);for(int i=0;i<n;i++){cin>>a;s.K(i);s.t->v=a;s.U(s.t);}cin>>m;while(m--){cin>>a>>l>>r;if(a==1)s.Rv(l-1,r-1);else{if(l==1&&r==n){s.K(n/2);cout<<s.t->A<<"\n";}else if(l==1){s.K(r);cout<<s.t->l->A<<"\n";}else if(r==n){s.K(l-2);cout<<s.t->r->A<<"\n";}else{s.I(l-1,r-1);cout<<s.t->r->l->A<<"\n";}}}}